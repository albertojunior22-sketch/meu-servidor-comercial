...
# URLs oficiais do servidor Render
BASE_URL = "https://pensare-api-teste.onrender.com"

URL_LOGIN = f"{BASE_URL}/login"
URL_SENHA = f"{BASE_URL}/alterar-senha"


def obter_tempo_inicializacao_pc():
    """Retorna o timestamp de quando o Windows foi ligado."""
    try:
        lib = ctypes.windll.kernel32
        ms = lib.GetTickCount64() if hasattr(lib, "GetTickCount64") else lib.GetTickCount()
...
def iniciar_sistema_seguranca():
    arquivo_credenciais = "session.json"
    boot_atual = obter_tempo_inicializacao_pc()
    ...
    # Verifica sessão salva no mesmo boot do Windows
    if os.path.exists(arquivo_credenciais):
        try:
            with open(arquivo_credenciais, "r", encoding="utf-8") as f:
                sessao = json.load(f)

            if abs(sessao.get("boot_time", 0) - boot_atual) < 60:
                res = requests.post(
                    URL_LOGIN,
                    json={
                        "email": sessao["email"],
    ...
    def acao_autenticar():
        ...
        if not email or not senha:
            messagebox.showerror("Erro", "Preencha todos os campos.", parent=janela_login)
            return

        try:
            btn_entrar.config(state="disabled", text="Verificando...")
            janela_login.update()
            ...
            if resposta.status_code == 200:
                ...
                if resultado.get("status") == "primeiro_acesso":
                    messagebox.showinfo(
                        "Primeiro Acesso",
                        resultado.get("mensagem", "Altere sua senha provisória."),
                        parent=janela_login
                    )

                    lbl_nova.pack(pady=(10, 0))
                    entry_nova.pack(fill="x", padx=40, pady=5)

                    btn_entrar.config(
                        text="Salvar Nova Senha",
                        command=lambda: salvar_nova_senha(email, senha),
                        state="normal"
                    )
                    return
                    ...
                    messagebox.showerror(
                        "Acesso Negado",
                        resultado.get("mensagem", "Acesso não autorizado."),
                        parent=janela_login
                    )
                    btn_entrar.config(state="normal", text="Entrar")
                    return
                ...
                try:
                    ...
                    detalhe = "Sem resposta do servidor."

                messagebox.showerror(
                    "Erro",
                    f"Servidor respondeu com erro.\n\nStatus: {resposta.status_code}\n\n{detalhe}",
                    parent=janela_login
                )
                btn_entrar.config(state="normal", text="Entrar")
            ...
            messagebox.showerror(
                "Erro de Conexão",
                f"Falha ao conectar ao servidor.\n\nMotivo técnico:\n{e}",
                parent=janela_login
            )
            btn_entrar.config(state="normal", text="Entrar")

    def salvar_nova_senha(email, senha_antiga):
        nova = entry_nova.get().strip()

        if not nova:
            messagebox.showerror("Erro", "Digite a nova senha.", parent=janela_login)
            return

        try:
            btn_entrar.config(state="disabled", text="Salvando...")
            janela_login.update()
            ...
            if res_s.status_code == 200:
                resultado = res_s.json()

                if resultado.get("status") == "sucesso":
                    messagebox.showinfo(
                        "Sucesso",
                        "Senha salva com sucesso! Entre novamente com a nova senha.",
                        parent=janela_login
                    )

                    entry_senha.delete(0, tk.END)
                    entry_nova.delete(0, tk.END)

                    entry_nova.pack_forget()
                    lbl_nova.pack_forget()

                    btn_entrar.config(
                        text="Entrar",
                        command=acao_autenticar,
                        state="normal"
                    )
                    return
                    ...
                    messagebox.showerror(
                        "Erro",
                        resultado.get("mensagem", "Não foi possível alterar a senha."),
                        parent=janela_login
                    )
                    btn_entrar.config(state="normal", text="Salvar Nova Senha")
                    return

            else:
                messagebox.showerror(
                    "Erro",
                    f"Servidor respondeu com erro.\n\nStatus: {res_s.status_code}\n\n{res_s.text}",
                    parent=janela_login
                )
                btn_entrar.config(state="normal", text="Salvar Nova Senha")
    ...
    # Janela de login
    janela_login = tk.Tk()
    janela_login.title("Acesso — Pensare Engenharia")
    janela_login.configure(bg="#1E2330")
    janela_login.geometry("380x360")
    janela_login.resizable(False, False)

    sw = janela_login.winfo_screenwidth()
    sh = janela_login.winfo_screenheight()
    janela_login.geometry(f"380x360+{(sw - 380) // 2}+{(sh - 360) // 2}")

    tk.Label(
        janela_login,
        text="⬡  AUTENTICAÇÃO DE LICENÇA",
    ...
    entry_senha.pack(fill="x", padx=40, pady=(5, 15))

    lbl_nova = tk.Label(
        janela_login,
        text="Defina sua nova senha pessoal:",
        font=("Segoe UI", 10),
        fg="#2ECC71",
        bg="#1E2330"
    )
    ...
    btn_entrar.pack(fill="x", padx=40, pady=15)
...
# Executa o bloco de segurança
iniciar_sistema_seguranca()


# ---------------------------------------------------------------------------
# Modo nuvem — serialização para API
# ---------------------------------------------------------------------------
USAR_NUVEM = True  # MVP: True envia leitura/processamento ao servidor

def _config_leitura_dict(estado):
    return {
        "unidade": estado.unidade,
        "em_distancia": estado.em_distancia,
        "dist_estaca": estado.dist_estaca,
        "dist_max_bloco": estado.dist_max_bloco,
        "fatores_hom": estado.fatores_hom or {},
    }


def _estado_para_payload_nuvem(estado):
    """Converte o EstadoProjeto em JSON simples para enviar ao servidor."""
    from projeto_json import _mapeamento_to_dict, _params_to_dict, _config_dist_to_dict
    try:
        from restricoes_ramo import restricoes_to_dict
    except Exception:
        restricoes_to_dict = None

    return {
        "nome": estado.nome,
        "tipo_projeto": estado._tipo_projeto,
        "config_leitura": _config_leitura_dict(estado),
        "mapeamento": _mapeamento_to_dict(estado.mapeamento),
        "params": _params_to_dict(estado.params),
        "config_dist": _config_dist_to_dict(estado.config_dist),
        "restricoes": restricoes_to_dict(estado.config_restricoes) if (estado.config_restricoes and restricoes_to_dict) else None,
    }
...
class Janela2Arquivos(JanelaBase):
    ...
    def _proximo(self):
        ...
        try:
            if USAR_NUVEM:
                import cliente_api
                dados = cliente_api.analisar_arquivos(
                    self.estado.arquivos,
                    _config_leitura_dict(self.estado)
                )
                self.estado.materiais_encontrados = dados.get("materiais", [])
                self.estado.ramos_nomes = dados.get("ramos", [])
                self.estado._projeto_lido = None
            else:
...
class Janela7Gerar(JanelaBase):
    ...
    def _gerar_worker(self, nome, pasta):
        ...
        try:
            if USAR_NUVEM:
                self._log("Enviando arquivos e configurações ao servidor...")
                import cliente_api
                payload = _estado_para_payload_nuvem(self.estado)
                saida = cliente_api.processar_distribuicao(
                    self.estado.arquivos,
                    payload,
                    pasta,
                    nome
                )
                caminho_excel = saida["excel"]
                caminho_json = saida["json"]
                self._log(f"  ✓ Excel: {caminho_excel}")
                self._log(f"  ✓ JSON: {caminho_json}")
                self._log("\n✅ Concluído com sucesso pelo servidor!")
                self._caminho_json = os.path.normpath(caminho_json)
                self.estado.caminho_json = os.path.normpath(caminho_json)

                def _habilitar_pos_nuvem():
                    reabilitar()
                    if self._btn_pos:
                        self._btn_pos.config(state="normal")

                self.after(0, _habilitar_pos_nuvem)
